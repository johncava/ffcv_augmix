        ...
        def corrupt_fixed(labs, _, inds):
            for i in parallel_range(labs.shape[0]):
                ...
                if np.random.rand() < 0.20:
                    ...
                    labs[i] = np.random.randint(low=0, high=1000)
            return labs
...
class Augmix(Operation):
    ...
    def __init__(self):
        ...
        self.to_device = ToDevice(ch.device(0), non_blocking=True)
        self.to_tensor = ToTensor()
        self.norm = NormalizeImage(IMAGENET_MEAN, IMAGENET_STD, np.float16)
    ...
    def generate_code(self):
        ...
        def augmix(images, dst):
            x1 = to_tensor(to_device(preprocess(x)))
            x2 = to_tensor(to_device(aug(x, preprocess)))
            x3 = to_tensor(to_device(aug(x, preprocess)))
            im_tuple = (x1,x2,x3)
            return im_tuple

        augmix.is_parallel = False
        # augmix.with_indices = True
        return augmix
...
class ImageNetTrainer:
    ...
    def train(self, epochs, log_level):
        ...
        if self.gpu == 0:
            ch.save(self.model.state_dict(), './final_weights.pt')
